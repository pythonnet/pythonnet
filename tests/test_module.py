...
def test_module_interface():
    ...
    assert isinstance(System.__dict__, dict)
    ...
    assert fnmatch(system_file, "*System*.dll") or fnmatch(
        system_file, "*mscorlib.dll"
    ), "unexpected System.__file__: " + system_file
    assert System.__doc__.startswith(
...
def test_clr_list_assemblies():
    ...
    assert "System" in short
    assert "Culture=" in verbose[0]
    assert "Version=" in verbose[0]
