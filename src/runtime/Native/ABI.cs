...
namespace Python.Runtime.Native
{
    static class ABI
    {
        ...
        internal static void Initialize(Version version)
        {
            ...
            if (IsFreeThreaded && version < new Version(3, 14))
            {
                // We need Py_REFCNT to be available as a symbol, which is only
                // present in Python 3.14 and later.
                throw new NotSupportedException($"Python ABI v{version} is not supported: Free-threaded builds are only supported on Python 3.14 and later.");
            }
            ...
            string className = Invariant($"Python.Runtime.TypeOffset{version.Major}{version.Minor}");
